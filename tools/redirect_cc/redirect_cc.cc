...
#include "vortex/tools/redirect_cc/gomacc_buildflags.h"
...
const base::FilePath::StringPieceType kVortexChromiumSrc =
    FILE_PATH_LITERAL("vortex/chromium_src");
...
class RedirectCC {
 public:
  ...
  int Run() {
    ...
    // Path to `src/vortex/chromium_src`.
    base::FilePath::StringType vortex_chromium_src_dir;
    // Path to `src/`.
    ...
    for (int arg_idx = first_compiler_arg_idx; arg_idx < argc_; ++arg_idx) {
      ...
      if (base::StartsWith(arg_piece, kIncludeFlag) &&
          base::EndsWith(arg_piece, kVortexChromiumSrc)) {
        arg_piece.remove_prefix(kIncludeFlag.size());
        vortex_chromium_src_dir = base::FilePath::StringType(arg_piece);
        arg_piece.remove_suffix(kVortexChromiumSrc.size());
    ...
    for (int arg_idx = first_compiler_arg_idx; arg_idx < argc_; ++arg_idx) {
      ...
      if (!compile_file_found && base::Contains(kCompileFileFlags, arg_piece)) {
        ...
        if (!path_cc.empty()) {
          base::FilePath::StringType vortex_path = base::StrCat(
              {vortex_chromium_src_dir, FILE_PATH_LITERAL("/"), path_cc});
          if (base::PathExists(base::FilePath(vortex_path))) {
            launch_argv.push_back(base::FilePath::StringType(arg_piece));
            launch_argv.push_back(std::move(vortex_path));
            ++arg_idx;
            continue;
          }
        }
