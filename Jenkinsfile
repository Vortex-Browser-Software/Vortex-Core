        ...
        stage('build') {
            ...
            steps {
                script {
                    ...
                    PIPELINE_NAME = 'pr-vortex-browser-' + CHANGE_BRANCH.replace('/', '-') + '-' + PLATFORM

                    withCredentials([usernamePassword(credentialsId: 'vortex-builds-github-token-for-pr-builder', usernameVariable: 'PR_BUILDER_USER', passwordVariable: 'PR_BUILDER_TOKEN')]) {
                        GITHUB_API = 'https://api.github.com/repos/vortex'
                        ...
                        def prDetails = readJSON(text: httpRequest(url: GITHUB_API + '/vortex-core/pulls?head=vortex:' + CHANGE_BRANCH_ENCODED, customHeaders: GITHUB_AUTH_HEADERS, quiet: true).content)[0]
                            ...
                            definition {
                                cpsScm {
                                    scm {
                                        git {
                                            remote {
                                                credentials('vortex-builds-github-token-for-pr-builder')
                                                github('vortex/devops', 'https')
                                            }
                                            branch('${params.DEVOPS_BRANCH}')
                                        }
                                    }
                                    scriptPath("jenkins/jobs/browser/pr-vortex-browser-${PLATFORM}.Jenkinsfile")
                                }
                            }
                        }
                    """)
