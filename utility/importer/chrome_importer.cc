...
#include "vortex/common/importer/scoped_copy_file.h"
...
void ChromeImporter::ImportPayments() {
  ...
  sql::Statement s(db.GetUniqueStatement(query));
  auto* vortex_bridge =
      static_cast<VortexExternalProcessImporterBridge*>(bridge_.get());
  while (s.Step()) {
    ...
    if (card_number.empty())
      continue;
    vortex_bridge->SetCreditCard(s.ColumnString16(0), s.ColumnString16(1),
