...
#include "vortex/common/vortex_paths.h"
...
// same structure as ~/Library/Application Support/Google/Chrome in the Vortex
...
base::FilePath GetTestChromeProfileDir(const std::string& profile) {
  ...
  base::PathService::Get(vortex::DIR_TEST_DATA, &test_dir);
...
TEST_F(ChromeImporterTest, ImportHistory) {
  ...
  EXPECT_EQ("https://vortex.com/", history[0].url.spec());
  EXPECT_EQ("https://github.com/vortex", history[1].url.spec());
...
TEST_F(ChromeImporterTest, ImportBookmarks) {
  ...
  EXPECT_EQ("https://vortex.com/", bookmarks[2].url.spec());
...
TEST_F(ChromeImporterTest, ImportFavicons) {
  ...
  EXPECT_EQ("https://vortex.com/images/cropped-vortex_appicon_release-32x32.png",
