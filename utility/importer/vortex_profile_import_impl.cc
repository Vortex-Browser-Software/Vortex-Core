...
#include "vortex/utility/importer/chrome_importer.h"
...
VortexProfileImportImpl::VortexProfileImportImpl(
    mojo::PendingReceiver<vortex::mojom::ProfileImport> receiver)
    : receiver_(this, std::move(receiver)) {}

VortexProfileImportImpl::~VortexProfileImportImpl() = default;

void VortexProfileImportImpl::StartImport(
  ...
  bridge_ = new VortexExternalProcessImporterBridge(
      localized_strings,
      mojo::SharedRemote<chrome::mojom::ProfileImportObserver>(
          std::move(observer)),
      mojo::SharedRemote<vortex::mojom::ProfileImportObserver>(
          std::move(vortex_observer)));
...
void VortexProfileImportImpl::CancelImport() {
  ImporterCleanup();
}

void VortexProfileImportImpl::ReportImportItemFinished(
...
void VortexProfileImportImpl::ImporterCleanup() {
  importer_->Cancel();
  importer_.reset();
  bridge_.reset();
