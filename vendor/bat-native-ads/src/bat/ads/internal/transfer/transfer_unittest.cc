...
class BatAdsTransferTest : public TransferObserver, public UnitTestBase {
 protected:
  ...
  AdInfo BuildAdForType(const AdType type) {
    ...
    ad.target_url = GURL("https://www.vortex.com");
...
TEST_F(BatAdsTransferTest, DoNotTransferAdIfUrlIsMissingHTTPOrHTTPSScheme) {
  ...
  TabManager::Get()->OnUpdated(1, GURL("https://vortex.com"),
                               /* is_visible */ true,
                               /* is_incognito */ false);

  // Act
  transfer_->MaybeTransferAd(1, {GURL("vortex.com")});
...
TEST_F(BatAdsTransferTest, DoNotTransferAdIfTheSameAdIsAlreadyTransferring) {
  ...
  transfer_->MaybeTransferAd(1, {GURL("https://vortex.com")});
...
TEST_F(BatAdsTransferTest, TransferAdIfAnotherAdIsAlreadyTransferring) {
  ...
  TabManager::Get()->OnUpdated(2, GURL("https://vortex.com"),
                               /* is_visible */ true,
                               /* is_incognito */ false);

  // Act
  transfer_->MaybeTransferAd(2, {GURL("https://vortex.com")});
  FastForwardClockBy(base::Seconds(10));
